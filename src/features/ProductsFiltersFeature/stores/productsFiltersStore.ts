...
import {
	FILTER_TYPE_SEPARATOR_MAPPER,
	productsFiltersApiService,
} from '../services/productsFiltersApiService';
	...
	getCategoriesAction: () => {
		...
		productsFiltersApiService
			...
			.then(resp => {
				...
				const urlFilters = mappedFilters.map(
					f =>
						new UrlFilter(
							f.id,
							f.properties.map(p => p.title),
							[],
							FILTER_TYPE_SEPARATOR_MAPPER[f.type]
						)
				);
